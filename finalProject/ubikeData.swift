...
class ubikeData: ObservableObject {
    @Published var BikeInfo = [ubikeInfo]()
    ...
    func fetchBike(){
        let query = "https://ptx.transportdata.tw/MOTC/v2/Bike/Availability/Taoyuan?$top=100&$format=JSON"
        if let urlStr = query.addingPercentEncoding(withAllowedCharacters:
            ...
            if let url = URL(string: urlStr) {
                ...
                URLSession.shared.dataTask(with: request) { (data, response, error) in
                    ...
                    if let data = data, let bikeResults = try? decoder.decode([ubikeInfo].self, from: data) {
                        self.BikeInfo = bikeResults
                    }
                    else{
                        print("error")
                    }
